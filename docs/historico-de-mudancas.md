...
- **Histórico WhatsApp em linguagem simples**: em `/admin/whatsapp`, tipos (`CONFIRMATION`, `BOT_REPLY`…) e erros Meta passam a título + orientação (`src/lib/whatsapp-log-labels.ts`).
- **Lembrete WhatsApp quando o horário está próximo**: além do aviso ~24h, envia lembrete ~2h antes. Cron HTTP `/api/cron/whatsapp-jobs` + `CRON_SECRET`.
- **Comanda no WhatsApp após agendar**: detalhes + link `/minha-reserva/{token}` (site, bot e agente Plus+).
...
- **Configurações profissionais**: `/admin/configuracao` acessível a todos os papéis. Seções **Aparência** (tema claro/escuro — casa do toggle; removido da sidebar), **Funções da barbearia** (marketplace, bot WhatsApp, Asaas, checklist “Primeiros passos” via `onboardingJson.hideChecklist`) e **Textos auxiliares** (OWNER). Componentes: `admin-config-appearance.tsx`, `admin-config-feature-toggles.tsx`.
- **Painel — modo claro**: chrome autenticado `/admin/(panel)` ganha tema claro opcional (`BN_LIGHT` + `.brand-onyx[data-theme="light"]`). Preferência em `localStorage` (`bn-admin-theme`) + cookie anti-flash; default escuro. Marketing/login/tenant públicos inalterados; canvas: só o chrome do studio segue o tema.
...
- **Admin mobile iPhone — menu vs canvas**: o studio fullscreen usava `top-14` fixo e, no Safari com notch, invadia o header e às vezes pintava por cima do drawer. Agora o topo segue a altura real do header (`--admin-mobile-top`); com o menu aberto o canvas fica oculto; drawer em z-index alto + safe-area.
- **Canvas — zoom com scroll como na main**: a coluna do palco ganhou `min-w-0` + `overflow-hidden` (o wrapper do empty-state quebrava o flex). Com zoom alto voltam as barras de rolagem internas em vez de cortar o painel direito.
- **Canvas `/admin/site` (A–D)**: chrome BN + `AdminPageHeader`; labels sem jargão (Desktop/Celular, Escurecer fundo, Ordem); modal BN no lugar de `window.confirm`; publicar com toast “Alterações no ar” e chip Não salvo/No ar; onboarding `canvasStudioSeen` (escolher modelo → marca → publicar); atalhos Delete/Ctrl+D/setas/Esc/Ctrl+S; painel Camadas; preview celular local; “Gerar versão celular” com confirmação. Componentes: `canvas-confirm-modal`, `canvas-layers-panel`, `canvas-onboarding`, `canvas-phone-preview`.
- **P1 continuidade da marca (painel `/admin`)**: chrome com `.brand-onyx` + Montserrat (`layout` do painel); sidebar agrupada (Visão geral / Operação / Marca & presença / Financeiro / Conta) em `admin-panel-nav.tsx`; cabeçalhos via `AdminPageHeader` (sem Bebas); checklist de onboarding e KPIs da visão geral com superfícies BN. Features profundas (canvas, WhatsApp, charts) intactas.
...
- **Clube completo para o cliente**: página pública `/{slug}/clube` + PIX/QR na adesão (admin e público); reset de `visitsUsed` a cada pagamento/renovação no webhook; link copiável no painel.
