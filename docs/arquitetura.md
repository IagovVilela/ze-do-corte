...
- **`/[slug]`** — Site institucional do tenant. `Organization.siteJson` **v2 (canvas)** renderizado por `TenantCanvasRenderer`. Fallbacks de branding neutros. Constantes `BARBER_*` **não** alimentam essa rota. **PWA:** manifest + botão **Instalar app** (atalho na tela inicial), espelhando o painel.
- **`/[slug]/agendar`** — Formulário cliente (`BookingForm`) scoped à org: busca/filtro de serviços (texto + categoria), seção **Mais pedidos** (`getPopularServiceIds` / `GET /api/appointments/popular-services`), multi-serviço e slots inteligentes. Também oferece **Instalar app** (mesmo PWA do site do tenant).
