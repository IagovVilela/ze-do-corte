...
- **`/[slug]`** — Site institucional do tenant. `Organization.siteJson` **v2 (canvas)** renderizado por `TenantCanvasRenderer`. Fallbacks de branding neutros. Constantes `BARBER_*` **não** alimentam essa rota. **PWA:** no celular, faixa **Baixar aplicativo** (atalho na tela inicial); no desktop, botão quando o Chrome oferecer.
- **`/[slug]/agendar`** — Formulário cliente (`BookingForm`) scoped à org: busca/filtro de serviços (texto + categoria), seção **Mais pedidos** (`getPopularServiceIds` / `GET /api/appointments/popular-services`), multi-serviço e slots inteligentes. No mobile, convite **Baixar aplicativo** no topo da seção e faixa fixa (mesmo PWA do site).
...
- **`/plataforma`** — console **Barbernegon Ops** isolado: entrada secreta `/plataforma/login?k=PLATFORM_OPS_GATE` (404 sem chave) → `GET /api/plataforma/gate` grava cookie httpOnly `bn_ops_gate` e remove o `k` da URL; e-mails em `PLATFORM_ADMIN_EMAILS` / `SEED_OWNER_EMAIL`; sidebar com visão geral, barbearias, marketplace, consumidores, suporte e consultores. Sem sessão → 404 (não revela login). APIs em `/api/plataforma/*`.
- **`/consultores`** — console da equipe de suporte: entrada `/consultores/login?k=SUPPORT_CONSULTANT_GATE`; contas `SUPPORT_CONSULTANT` criadas pelo Ops; assistência `SUPPORT_ASSIST` no painel do salão sem dados sensíveis. APIs `/api/consultores/*`.
...
- **`/admin/perfil`** — utilizador autenticado altera `displayName`, `phone`, senha (`PATCH /api/auth/profile`) e foto (`POST`/`DELETE /api/auth/profile/avatar` → Cloudinary quando configurado); a foto alimenta o cartão na home para quem é barbeiro em destaque. Opcionalmente ativa **Web Push** (VAPID + `public/sw.js`) para receber aviso de agendamento atribuído sem e-mail. O painel também é instalável como app (`/admin-manifest.webmanifest` + botão **Instalar app** no rodapé).
- **`/admin/configuracao`** — preferências do painel: **Aparência** (tema claro/escuro, todos os papéis); **Funções** (marketplace, bot WhatsApp, Asaas, ocultar checklist — OWNER); **Textos auxiliares** (`BarbershopSetting` — OWNER).
...
- **`/admin` (painel)** — No topo (OWNER/ADMIN): **briefing matinal** ranqueado com chip **Faça agora** e CTA para **`/admin/inteligencia` (Braço Direito)** — análise comparativa, gráficos e insights (não mais só texto genérico no dashboard). Gráficos temporais agrupam por **calendário em `America/Sao_Paulo`** (`BARBER_TIMEZONE`), não pelo fuso do servidor. `getAdminDashboardSnapshot` (mesmos filtros de URL que a lista: `status`, `staff`, `unit`, `q`) + gráficos + **Resumo operacional** com formulário de filtros; **OWNER/ADMIN** têm **telemetria por unidade** abaixo das abas de período, com janela **Hoje e semana** ou **Período dos gráficos** (`?telemetryScope=chart`), agregada por `unitId`; com filtro de unidade na URL, só essa unidade aparece. Tabela de agendamentos: atribuir barbeiro, **remarcar** data/hora, concluir/cancelar, pagamento — cancelar/remarcar avisam o cliente (WhatsApp + e-mail). `src/app/admin/(panel)/layout.tsx`: `getStaffAccessOrNull()`; sem sessão → `redirect("/admin/login")`. Chrome BN com **modo claro/escuro** (`AdminThemeProvider`; default escuro; `localStorage` + cookie `bn-admin-theme`).
- **Financeiro do salão** (`viewRevenue`) — `/admin/financeiro/comissoes`, `/balanco`, contas, criar despesa/receita; **gerencial Pro**: `/configuracao`, `/precificacao`, `/dre`, `/ponto-equilibrio`, `/fluxo-caixa` — ver [financeiro-gerencial.md](./financeiro-gerencial.md). APIs `/api/admin/finance/*`; libs `admin-finance.ts`, `admin-commissions.ts`, `finance-settings.ts`, `service-costing.ts`, `finance-dre.ts`, `finance-break-even.ts`, `finance-cashflow.ts`. No mobile: listas em cards. **Caixa** (`/admin/caixa`) = relatório de atendimentos pagos (Pro).
- **Evolução** — `/admin/evolucao` + `GET /api/admin/evolution` (`admin-evolution.ts`): faturamento mensal, taxas de retorno, sem preferência, crescimento por unidade/clube; painel responsivo (filtros empilhados, gráficos e chips com scroll no celular).
