...
- **`/[slug]/agendar`** — Formulário cliente (`BookingForm`) scoped à org: busca/filtro de serviços (texto + categoria), seção **Mais pedidos** (`getPopularServiceIds` / `GET /api/appointments/popular-services`), multi-serviço e slots inteligentes.
