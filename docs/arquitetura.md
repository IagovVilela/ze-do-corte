...
- **Financeiro do salão** (`viewRevenue`) — `/admin/financeiro/comissoes`, `/balanco`, `/contas-a-pagar`, `/contas-a-receber`, `/criar-despesa`, `/criar-receita`; APIs `/api/admin/finance/*`; libs `admin-finance.ts` / `admin-commissions.ts`. No mobile: listas em cards (tabela a partir de `md`/`lg`). **Caixa** (`/admin/caixa`) segue como relatório de atendimentos pagos (Pro).
