...
export function ConsultantLoginForm({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setPending(true);
    try {
      const res = await fetch("/api/consultores/login", {
        method: "POST",
      ...
      if (!res.ok) {
        setError(
          res.status === 404
            ? "Link expirado. Abra de novo a URL com ?k=…"
            : (data.message ?? "Não foi possível entrar."),
        );
        return;
      }
      router.push(data.redirect || "/consultores");
      router.refresh();
    } catch {
    ...
    >
      <div>
        ...
        <h1 className="mt-2 text-xl font-semibold text-white">
          Console de consultores
        </h1>
        <p className="mt-1 text-sm text-zinc-400">
          Suporte aos salões — sem acesso financeiro ou chaves.
        </p>
      </div>
